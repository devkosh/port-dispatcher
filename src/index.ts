...
program
  ...
  .action((portStr: string, opts: { count: string }) => {
    const port = parseInt(portStr, 10);
...
program
  ...
  .action(async (portStr: string, opts: { force: boolean; yes: boolean }) => {
    const port = parseInt(portStr, 10);
    if (isNaN(port) || port < 0 || port > 65535) {
      console.error('Invalid port number (0–65535)');
      process.exit(1);
    ...
    if (!opts.yes) {
      ...
      const ok = await confirm(
        `  Kill ${chalk.yellow(entry.process)} (pid ${entry.pid}) with ${chalk.yellow(sig)}? ${chalk.dim('[y/N]')} `,
      );
      if (!ok) {
