...
export function scan(ranges = DEFAULT_RANGES, freeCount = 5) {
    const t0 = Date.now();
    const allBusy = getAllListeningPorts();
    const rangeResults = ranges.map((range) => {
        const busyPorts = [];
        const nextFreePorts = [];
